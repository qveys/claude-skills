...
- **Boot steps do NOT survive an image update** (they live inside the image, unlike the payloads on the volume). After any image pull/rebuild: `docker exec paperclip-paperclip-1 ls /opt/paperclip/entrypoint.d/` and reinstall any missing hook (e.g. `42-openclaw-protocol-v4.sh`) from its volume payload before considering the update done.
