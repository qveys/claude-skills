...
- En-tête de fiche : `Phase X · après Y · **Modèle : Sonnet|Opus|Haiku|Fable**` — le relais lit cette ligne pour choisir le modèle (Fable = le plus capable, pour les fiches de jugement les plus exigeantes). Défaut Sonnet ; gros modèle uniquement pour les fiches de jugement (audit, arbitrage qui redécoupe le plan) ; escalade ponctuelle sur blocage réel, consignée dans STATE.md.
- Le titre de `STATE.md` (`# STATE — chantier <NOM>`) donne son **nom** au chantier : le relais l'extrait pour le kickoff de chaque session (repli : nom du dossier projet).
